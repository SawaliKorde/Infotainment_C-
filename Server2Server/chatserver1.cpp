...
#define PORT_A 8080  //Home address
#define PORT_B 8083  //Server 2 address
   ...
   void handleClient(int client_socket, string message)
   {
	...
	if(client_map.empty()){
	  ...
	  cout<<"New client connected: "<<endl;
		...
		string R_clientNo = "Client " + to_string(client_socket) + " says: ";
      while (true) {
	...
	regex messageRegex("^\\d+:.+$");
        ...
        try{
	 	...
	 	for (const auto& client: client_map)
	 	{
		    ...
		    string sendmessage = R_clientNo + message.substr(message.find(":") + 1); // should save message of client
