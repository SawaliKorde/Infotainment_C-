...
#define DEVICE_NAME "flowm_dev"
#define TIMER_INTERVAL 3 * HZ

static int major;
static void *shm_region_flow;
...
static void generate_random_data(struct timer_list *t) {
    ...
    entry = (struct data_entry *)((char *)shm_region_flow + index *
sizeof(struct data_entry));
    entry->timestamp = jiffies;
    entry->value = 40 + (get_random_u64() % 71);  // Generate random


    pr_info("Current flowmeter is %d gpm | Timestamp: %lu\n",entry->value, entry->timestamp);
...
static int device_mmap(struct file *file, struct vm_area_struct *vma) {
    ...
    unsigned long pfn = virt_to_phys(shm_region_flow) >> PAGE_SHIFT;
...
static int __init device_init(void) {
    ...
    shm_region_flow = (void *)__get_free_pages(GFP_KERNEL, get_order(SHM_SIZE));
if (!shm_region_flow) {
    pr_err("Failed to allocate shared memory for flowmeter data\n");
    return -ENOMEM;
}
...
static void __exit device_exit(void) {
    ...
    free_pages((unsigned long)shm_region_flow, get_order(SHM_SIZE));
