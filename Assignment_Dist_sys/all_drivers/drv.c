...
#define DEVICE_NAME "temp_dev"
#define TIMER_INTERVAL 3 * HZ

static int major;
static void *shm_region_temp;
...
static void generate_random_data(struct timer_list *t) {
    ...
    entry = (struct data_entry *)((char *)shm_region_temp + index *
sizeof(struct data_entry));
    entry->timestamp = jiffies;
    entry->value = 15 + (get_random_u64() % 61);  // Generate random


    pr_info("Current temperature is %d deg | Timestamp: %lu\n",entry->value, entry->timestamp);
...
static int device_mmap(struct file *file, struct vm_area_struct *vma) {
    ...
    unsigned long pfn = virt_to_phys(shm_region_temp) >> PAGE_SHIFT;
...
static int __init device_init(void) {
    ...
    shm_region_temp = (void *)__get_free_pages(GFP_KERNEL, get_order(SHM_SIZE));
    if (!shm_region_temp) {
...
static void __exit device_exit(void) {
    ...
    free_pages((unsigned long)shm_region_temp, get_order(SHM_SIZE));
    unregister_chrdev(major, DEVICE_NAME);
