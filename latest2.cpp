...
public:
    ...
    DataOperation(const DataOperation& other) : fileMutex(other.fileMutex) {} // 
...
struct Table: public DataOperation {
    ...
    Table() = default; 	
    Table(mutex& mutex) : DataOperation(mutex) {}

    void create(const string& name) override { insert(vector<string>(), fileMutex); }
   
    void remove(const string& name) override { delete_record("column", "value", fileMutex); }
    ...
    void save_to_csv(mutex& fileMutex) {
        ...
        lock_guard<mutex> lock(fileMutex);
        try {
            string savefilepath = dbpath + name + ".csv";
            ...
            for (const auto& record : rows) {
                for (size_t i = 0; i < column_names.size(); ++i) {
                    file << record.columns.at(column_names[i]);
                    if (i < column_names.size() - 1) file << ",";
                }
                file << "\n";
            }
    ...
    void load()  {
        ...
        lock_guard<mutex> lock(fileMutex);
        try {
            string filepath = dbpath + name + ".csv";
            ...
            cerr << "Error loading from CSV: " << e.what() << endl;
        }
    }
};
...
public:
    ...
    void create(const string& name) override {
        vector<string> empty;
        create_table(name, empty, "db_name", fileMutex);
    }

    void select(const string& name)  {
        if (auto table = get_table(name)) {
...
public:
    DBMS() : pool(4) {current_db = nullptr};
...
int main() {
    ...
    thread saveThread([&dbms, &savePromise, &fileMutex]() {
        try {
            // Perform save operation
            for (auto& db : dbms.databases) {
                db.second.save();
            }
