...
struct Table {
    ...
    string file_path = folder_path + name + ".csv";

    // Launch a background thread for saving
    thread save_thread([=]() {
        try {
            ...
            this_thread::sleep_for(std::chrono::seconds(10)); //Simulate a delay
            ...
            file.close();
            cout << "Table saved successfully to " << file_path << endl;
    ...
    // Notify the user that saving is in progress
    cout << "Saving table to CSV in the background...\n";
}
    };
