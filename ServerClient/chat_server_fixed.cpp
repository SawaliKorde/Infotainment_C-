...
void handleClient(int client_socket) {
    ...
    try {
        while (true) {
            ...
            // Automatic server response for this specific client
            cout <<"Server response to client "<< endl;
            string response;
            getline(cin,response); 
            if (send(client_socket, response.c_str(), response.length(), 0) < 0) {
                cout << "Failed to send message to client " << client_socket << endl;
                break;
            }
    }catch (const exception& e) {
        cout << "Error Handling Client " << client_socket << ": " << e.what() << endl;
    }
