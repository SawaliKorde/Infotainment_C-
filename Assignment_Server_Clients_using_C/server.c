...
void* generateData(void* arg) {
    int index = 0; // Index to track the current position in shared memory

    while (true) {
        struct sensorData data;
        data.bus_id = rand() % 10;
        data.dev_id = rand() % 3; // Randomly choose a device type

        switch (data.dev_id) {
            case 0: // Temp
                data.scale = rand() % 500;
                data.raw_data = getRandomTemp();
                break;
            case 1: // Pressure
                data.scale = rand() % 250;
                data.raw_data = getRandomPressure();
                break;
            case 2: // Voltage
                data.scale = rand() % 1000;
                data.raw_data = getRandomVoltage();
                break;
        }

        // Write data to shared memory
        sem_wait(&mem); // Lock the semaphore
        memcpy((struct sensorData*)ptr1 + index, &data, sizeof(struct sensorData));
        printf("DATA WRITTEN T SHARED MEM\n");
        sem_post(&mem); // Unlock the semaphore

        index++;
        if (index >= shm_size / sizeof(struct sensorData)) {
            index = 0; // Reset index when memory is full
            printf("Shared memory full. Resetting index.\n");
        }
